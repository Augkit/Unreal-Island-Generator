                                     ...
                                     float Temperature, float Moisture)
{
	...
	for (FBiomeData biome : possibleBiomes)
	{
		if ((biome.MinMoisture < Moisture || biome.MinMoisture == 0.0f && Moisture == 0.0f) && biome.MaxMoisture >=
			Moisture)
		{
			newPossibleBiomes.Add(biome);
		}
	...
	{
		UE_LOG(LogMapGen, Error,
		       TEXT("Could not find any biomes with moisture %f! Is ocean? %d Is coast? %d Is water? %d"), Moisture,
		       (uint8)bIsOcean, (uint8)bIsCoast, (uint8)bIsWater);
		return FBiomeData();
	}
	newPossibleBiomes.Empty();
	...
	for (FBiomeData biome : possibleBiomes)
	{
		if ((biome.MinTemperature < Temperature || biome.MinTemperature == 0.0f && Temperature == 0.0f) && biome.
	...
	else if (possibleBiomes.Num() == 0)
	{
		UE_LOG(LogMapGen, Error,
		       TEXT(
			       "Could not find any biomes with temperature %f! Is ocean? %d; Is coast? %d; Is water? %d; Moisture: %f"
		       ), Temperature, (uint8)bIsOcean, (uint8)bIsCoast, (uint8)bIsWater, Moisture);
		return FBiomeData();
	}

	UE_LOG(LogMapGen, Warning, TEXT("Had %d possible candidates for temperature %f and moisture %f."),
	       possibleBiomes.Num(), Temperature, Moisture);
...
void UIslandMapUtils::DrawDelaunayMesh(AActor* Context, UTriangleDualMesh* Mesh, const TArray<float>& RegionElevations,
                                       ...
                                       const TArray<float>& TriangleElevations, const TArray<FBiomeData>& RegionBiomes)
{
	...
	for (FSideIndex e = 0; e < _halfedges.Num(); e++)
	{
		if (e < _halfedges[e])
		{
			...
			FLinearColor color = FMath::Lerp(RegionBiomes[first].DebugColor.ReinterpretAsLinear(),
			                                 RegionBiomes[second].DebugColor.ReinterpretAsLinear(), 0.5f);
...
void UIslandMapUtils::DrawVoronoiMesh(AActor* Context, UTriangleDualMesh* Mesh, const TArray<FIslandPolygon>& Polygons,
                                      const TArray<int32>& SideFlow, const TArray<URiver*>& Rivers,
...
void UIslandMapUtils::DrawRivers(AActor* Context, UTriangleDualMesh* Mesh, const TArray<URiver*>& Rivers,
...
void UIslandMapUtils::GenerateMapMeshSingleMaterial(UTriangleDualMesh* Mesh, UProceduralMeshComponent* MapMesh,
                                                    float ZScale, const TArray<float>& RegionElevation)
{
	...
	MapMesh->CreateMeshSection_LinearColor(0, meshData.Vertices, meshData.Triangles, meshData.Normals, meshData.UV0,
	                                       meshData.VertexColors, meshData.Tangents, true);

	// Enable collision data
	MapMesh->ContainsPhysicsTriMeshData(true);
}

void UIslandMapUtils::GenerateMapMeshMultiMaterial(UTriangleDualMesh* Mesh, UProceduralMeshComponent* MapMesh,
                                                   float ZScale, const TArray<float>& RegionElevation,
                                                   const TArray<bool>& CostalRegions,
                                                   const TArray<FBiomeData> RegionBiomes)
{
	...
	for (auto kvp : meshLookup)
	{
		...
		MapMesh->CreateMeshSection_LinearColor(index, meshData.Vertices, meshData.Triangles, meshData.Normals,
...
void UIslandMapUtils::TriangulateContour(const FAreaContour& Contour, TArray<FPolyTriangle2D>& Triangles)
{
	TArray<int32> Indices;
	Indices.Empty(Contour.Indices.Num());
	for (const FTriangleIndex& Index : Contour.Indices)
	{
		Indices.Add(Index);
	}
