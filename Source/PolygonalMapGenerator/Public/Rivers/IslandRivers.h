...
protected:
	...
	*/
	...
	virtual TArray<URiver*> CreateRiver(FTriangleIndex RiverTriangle, TArray<int32>& s_flow,
	...
	virtual void GenerateRiverPolygons(UTriangleDualMesh* Mesh, TArray<FRiverPolygon>& RiverPolygons,
	                                   const TArray<URiver*>& Rivers) const;

	virtual TArray<FTriangleIndex> FindSpringTriangles_Implementation(UTriangleDualMesh* Mesh,
	                                                                  const TArray<bool>& r_water,
	                                                                  const TArray<float>& t_elevation,
	                                                                  const TArray<FSideIndex>& t_downslope_s) const;
	virtual void AssignSideFlow_Implementation(TArray<int32>& s_flow, TArray<URiver*>& Rivers,
...
public:
	...
	*/
	...
	TArray<FTriangleIndex> FindSpringTriangles(UTriangleDualMesh* Mesh, const TArray<bool>& WaterRegions,
	                                           const TArray<float>& TriangleElevations,
	                                           const TArray<FSideIndex>& TriangleSideDownslopes) const;
	UFUNCTION(BlueprintCallable, BlueprintNativeEvent, Category = "Procedural Generation|Island Generation|Rivers")
	void AssignSideFlow(UPARAM(ref) TArray<int32>& SideFlow, UPARAM(ref) TArray<URiver*>& Rivers,
	                    UPARAM(ref) TArray<FRiverPolygon>& RiverPolygons, UTriangleDualMesh* Mesh,
	                    const TArray<FSideIndex>& TriangleSideDownslopes, const TArray<FTriangleIndex>& RiverTriangles,
	                    UPARAM(ref) FRandomStream& RiverRNG) const;

	TArray<FTriangleIndex> find_spring_t(UTriangleDualMesh* Mesh, const TArray<bool>& r_water,
	                                     const TArray<float>& t_elevation,
	                                     const TArray<FSideIndex>& t_downslope_s) const;
	void assign_s_flow(TArray<int32>& s_flow, TArray<URiver*>& Rivers, TArray<FRiverPolygon>& RiverPolygons,
